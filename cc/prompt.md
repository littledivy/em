...
6. Do NOT git commit, push, or open PR for the INITIAL fix — orchestrator does that. (For follow-up fixes after a PR exists and you've been re-engaged on review feedback, you SHOULD `git add -A && git commit -m "<msg>" && git push bot HEAD` immediately. Orchestrator's push is a no-op then.) After signaling done, your session will be killed; don't try to watch CI yourself — the orchestrator polls every tick and re-engages you on failure/comment/conflict.
